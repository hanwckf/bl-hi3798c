...
static int higmac_mdio_read(struct higmac_netdev_local *ld,
	char *devname, unsigned char phy, unsigned char reg, unsigned short *value)
{
	int timeout = 1000;
...
int higmac_mdiobus_write(struct mii_dev *bus, int addr, int devad, int reg, u16 value)
{
	struct higmac_netdev_local *ld = higmac_get_netdev_by_name(bus->name);
	...
	return higmac_mdio_write(ld, bus->name, addr, reg, value);
}
